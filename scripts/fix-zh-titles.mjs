...
async function main() {
  ...
  for (const file of files) {
    ...
    const needTranslation = items.filter(i => isPureEnglish(i.title));

    if (needTranslation.length === 0) {
      console.log(`${file}: no untranslated entries`);
      continue;
    }

    console.log(`${file}: ${needTranslation.length} entries need translation`);
