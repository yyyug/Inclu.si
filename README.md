...
Daily and weekly digest content is read from per-year JSON files under `src/data/digests/`.

### Daily digests

File: `src/data/digests/daily-YYYY.json`

Keys are `YYYY-MM-DD` date strings. Each record:

```json
{
  "generatedAt": "2026-07-31T07:51:45.701Z",
  "en": {
...
File: `src/data/digests/weekly-YYYY.json`

Keys are ISO week strings like `2026-W31`. Weekly highlights are merged from the week's daily digests (rule-based dedupe/rank), and the title + summary are generated by a single LLM pass per week.

```json
{
  "week": "2026-W31",
  "range": { "start": "2026-07-27", "end": "2026-08-02" },
  "generatedAt": "2026-08-03T01:20:00.000Z",
  "en": { "title": "...", "summary": "...", "highlights": [...] },
  "zh-TW": { "title": "...", "summary": "...", "highlights": [...] }
}
```

The homepage renders the latest daily digest. Dedicated pages at `/en/digests/YYYY-MM-DD` and `/en/digests/week/YYYY-Www` allow browsing past digests with previous/next navigation (View Transitions enabled).
...
- Schedule: twice daily at `00:37` and `12:37` UTC
- Action: runs `npm run news:digest`
- Output: upserts `src/data/digests/daily-YYYY.json`, migrates legacy `daily-digest.json` if present

### 2a) Weekly digest

File: `.github/workflows/weekly-digest.yml`

- Schedule: Monday at `01:20` UTC (also `workflow_dispatch` with optional `week` input)
- Action: runs `npm run news:weekly-digest`
- Output: merges the previous week's daily digests (rule-based), then a single LLM pass for title + summary; writes to `src/data/digests/weekly-YYYY.json`
